// app.js - Relatorio de Cumprimento de Sentenca
// Frontend JavaScript para o sistema de relatorio de cumprimento
...
class RelatorioCumprimentoApp {
    ...
    atualizarStatusEtapa(etapa, status) {
        ...
        if (status === 'ativo') {
            ...
            iconEl.querySelector('i').classList.remove('text-gray-400', 'text-white');
            iconEl.querySelector('i').classList.add('text-white');
            ...
            iconEl.querySelector('i').classList.remove('text-gray-400');
            iconEl.querySelector('i').classList.add('text-white');
    ...
    resetarStatusEtapas() {
        for (let i = 1; i <= 5; i++) {
            ...
            iconEl.querySelector('i').classList.remove('text-white');
            iconEl.querySelector('i').classList.add('text-gray-400');
    ...
    exibirEditor(data, isNova = true) {
        ...
        document.getElementById('editor-cnj').textContent = this.dadosCumprimento.numero_processo_formatado || this.numeroCNJ;
    ...
    abrirEditorStreaming() {
        document.getElementById('editor-cnj').textContent = this.numeroCNJ || 'Gerando...';
        document.getElementById('minuta-content').innerHTML = '<div class="animate-pulse"><div class="h-4 bg-gray-200 rounded w-3/4 mb-4"></div><div class="h-4 bg-gray-200 rounded w-1/2"></div></div>';
        this.abrirModal('modal-editor');
    }
    ...
    async enviarMensagemChat() {
        ...
        try {
            ...
            if (data.status === 'sucesso') {
                ...
                this.adicionarMensagemChat(`Nao consegui fazer a alteracao: ${data.mensagem}`, 'assistente');
            }
        ...
        if (tipo === 'usuario') {
            div.innerHTML = `
                <div class="chat-bubble-user px-4 py-3 max-w-[85%]">
                    <p class="text-sm text-white">${mensagem}</p>
                </div>
            `;
        } else {
            div.innerHTML = `
    ...
    abrirModalDocumentos() {
        ...
        document.getElementById('modal-docs-info').textContent =
            `${this.documentosBaixados.length} documento(s) analisado(s) - Processo ${numeroCumprimento}`;
        ...
        if (documentosVisiveis.length === 0) {
            ...
            lista.innerHTML = documentosVisiveis.map((doc, index) => {
                ...
                return `
                <button
                    onclick="app.visualizarDocumento('${doc.id_documento}', ${index}, '${numeroProcesso}')"
                ...
                >
                    <div class="flex items-center gap-2">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-medium text-gray-800 truncate">${nomeDoc}</p>
                            <p class="text-xs text-gray-500 truncate">${doc.id_documento}</p>
                        </div>
                        ${badge}
                    </div>
                </button>
        ...
        container.innerHTML = historico.map(h => `
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-medium text-gray-800 text-sm">${h.numero_cumprimento_formatado || h.numero_cumprimento}</p>
                        <p class="text-xs text-gray-500">${h.dados_basicos?.cumprimento?.autor || 'Autor nao identificado'}</p>
                    </div>
                </div>
    ...
    renderizarListaHistorico(historico) {
        ...
        container.innerHTML = historico.map(h => `
            <div class="p-3 border border-gray-100 rounded-xl mb-3 hover:border-primary-200 hover:bg-primary-50/30 transition-colors cursor-pointer"
                 onclick="app.carregarDoHistorico(${h.id})">
                <p class="font-medium text-gray-800 text-sm">${h.numero_cumprimento_formatado || h.numero_cumprimento}</p>
                <p class="text-xs text-gray-500 mt-1">${h.dados_basicos?.cumprimento?.autor || 'Autor nao identificado'}</p>
        ...
        return new Promise((resolve) => {
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
                    ...
                    <div class="flex gap-3">
                        <button class="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-colors" onclick="this.closest('.fixed').remove(); window.confirmResult('cancelar')">
                            Cancelar
                        </button>
                        <button class="flex-1 px-4 py-2 bg-blue-100 text-blue-700 rounded-xl hover:bg-blue-200 transition-colors" onclick="this.closest('.fixed').remove(); window.confirmResult('ver')">
                            Ver existente
                        </button>
                        <button class="flex-1 px-4 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition-colors" onclick="this.closest('.fixed').remove(); window.confirmResult('refazer')">
                            Refazer
                        </button>
                    </div>
                </div>
