...
// Built at: 2026-01-28T12:56:49.958Z
...
(() => {
  ...
  async function confirmReplaceFile() {
    ...
    if (win.pendingReplaceFile) {
      ...
      win.pendingReplaceFile = void 0;
    }
  }
  function cancelReplaceFile() {
    const modal = document.getElementById("duplicate-confirm-modal");
    if (modal) modal.remove();
    window.pendingReplaceFile = void 0;
  ...
  function startAnalysisPolling(fileId) {
    ...
    setTimeout(() => {
      if (appState.pollingIntervals[fileId]) {
        ...
        const statusEl2 = document.getElementById("analysis-status");
        if (statusEl2) {
          statusEl2.innerHTML = '<i class="fas fa-exclamation-triangle text-yellow-500"></i> Timeout na analise';
          statusEl2.className = "text-xs text-yellow-600 text-center mt-2";
        }
      }
    }, 3e5);
  }
  ...
  function printReportPdf() {
    ...
    <body>
      <div class="header">
        ...
        <p>Data: ${(/* @__PURE__ */ new Date()).toLocaleDateString("pt-BR")}</p>
      </div>
  ...
  function renderMarkdown(text) {
    return text.replace(/^### (.*$)/gim, '<h3 class="text-lg font-semibold mt-4 mb-2">$1</h3>').replace(/^## (.*$)/gim, '<h2 class="text-xl font-bold mt-6 mb-3 text-primary-700">$1</h2>').replace(/^# (.*$)/gim, '<h1 class="text-2xl font-bold mt-6 mb-4 text-primary-800">$1</h1>').replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>").replace(/\*([^*]+)\*/g, "<em>$1</em>").replace(/^- (.*$)/gim, '<li class="ml-4">$1</li>').replace(/^• (.*$)/gim, '<li class="ml-4">$1</li>').replace(/\n/gim, "<br>");
  }
  ...
  function showToast(message, type = "info") {
    ...
    toast.className = `fixed bottom-4 right-4 z-50 px-6 py-3 rounded-lg shadow-lg transition-all duration-300 flex items-center gap-2 ${type === "success" ? "bg-green-500 text-white" : type === "error" ? "bg-red-500 text-white" : type === "warning" ? "bg-yellow-500 text-white" : "bg-blue-500 text-white"}`;
    const icons = {
