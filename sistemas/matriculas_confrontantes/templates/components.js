 ...
 * @param {string} variant - Badge variant (success, warning, info, error)
...
function Badge(text, variant = 'info') {
    ...
    return `<span class="px-2 py-1 text-xs font-medium rounded-full ${variants[variant] || variants.info}">${text}</span>`;
}
 ...
 * @returns {string} HTML string
 */
function Button({ text, icon, variant = 'primary', size = 'md', onClick = '' }) {
    const variants = {
 ...
 * @returns {string} HTML string
 */
function IconButton({ icon, title = '', variant = 'ghost', onClick = '' }) {
    const variants = {
 ...
 * @returns {string} HTML string
 */
function DetailCard({ label, value, icon, badge = false, progress = null }) {
 ...
 * @returns {string} HTML string
 */
function Modal({ id, title, content, footer = '', size = 'md' }) {
    const sizes = {
        sm: 'max-w-md',
        md: 'max-w-lg',
        lg: 'max-w-2xl',
